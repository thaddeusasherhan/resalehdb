# Analysis for QUEENSTOWN
import pandas as pd
import matplotlib.pyplot as plt

def analyze_town_data(df, town_name='QUEENSTOWN'):
    ...
    plt.title(f'Price per Sqm Range with Median by Month - QUEENSTOWN')
    plt.xlabel('Month')
    plt.ylabel('Price per Sqm')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f"queenstown_analysis.png")
    plt.close()
