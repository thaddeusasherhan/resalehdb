...
#For each transaction, calculate price/sqm
    ...
    def tabulate_monthly_median_price_per_sqm(fourfive_filtered_df):
        # Ensure month is datetime and create year_month
        ...
        monthly_grouped = fourfive_filtered_df.groupby(['town', 'year_month']).agg(
            Median_Price_per_Sqm=('price_per_sqm', 'median')
        ).reset_index()

        # Pivot to get a tabular format: rows = towns, columns = year_month
        pivot_table = monthly_grouped.pivot(
            index='town',
            columns='year_month',
            values='Median_Price_per_Sqm'
        )

        # Optional: sort by town name
        ...
        pivot_table.to_csv('monthly_median_price_per_sqm_by_town.csv')

    tabulate_monthly_median_price_per_sqm(fourfive_filtered_df)
