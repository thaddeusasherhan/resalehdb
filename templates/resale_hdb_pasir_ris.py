# Analysis for PASIR RIS
import pandas as pd
import matplotlib.pyplot as plt

def analyze_town_data(df, town_name='PASIR RIS'):
    ...
    plt.title(f'Price per Sqm Range with Median by Month - PASIR RIS')
    plt.xlabel('Month')
    plt.ylabel('Price per Sqm')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f"pasir_ris_analysis.png")
    plt.close()
