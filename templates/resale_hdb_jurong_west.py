# Analysis for JURONG WEST
import pandas as pd
import matplotlib.pyplot as plt

def analyze_town_data(df, town_name='JURONG WEST'):
    ...
    plt.title(f'Price per Sqm Range with Median by Month - JURONG WEST')
    plt.xlabel('Month')
    plt.ylabel('Price per Sqm')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f"jurong_west_analysis.png")
    plt.close()
