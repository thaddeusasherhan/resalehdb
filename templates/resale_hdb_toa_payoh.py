# Analysis for TOA PAYOH
import pandas as pd
import matplotlib.pyplot as plt


def analyze_town_data(df, town_name='TOA PAYOH'):
    ...
    grouped = town_df.groupby('year_month').agg(
        ...
        Price_Std=('price_per_sqm', 'std')).reset_index()
    ...
    for _, row in grouped.iterrows():
        ...
        # Plot error bars
        plt.errorbar(x=row['year_month_str'],
    ...
    plt.title(f'Price per Sqm Range with Median by Month - TOA PAYOH')
    plt.xlabel('Month')
    ...
    return fig
    #plt.savefig(f"toa_payoh_analysis.png")
    #plt.close()
...
if __name__ == "__main__":
    ...
    filtered_df['price_per_sqm'] = filtered_df['resale_price'] / filtered_df[
        'floor_area_sqm']
