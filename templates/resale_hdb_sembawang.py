# Analysis for SEMBAWANG
import pandas as pd
import matplotlib.pyplot as plt

def analyze_town_data(df, town_name='SEMBAWANG'):
    ...
    plt.title(f'Price per Sqm Range with Median by Month - SEMBAWANG')
    plt.xlabel('Month')
    plt.ylabel('Price per Sqm')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f"sembawang_analysis.png")
    plt.close()
