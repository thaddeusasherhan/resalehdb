# Analysis for WOODLANDS
import pandas as pd
import matplotlib.pyplot as plt

def analyze_town_data(df, town_name='WOODLANDS'):
    ...
    plt.title(f'Price per Sqm Range with Median by Month - WOODLANDS')
    plt.xlabel('Month')
    plt.ylabel('Price per Sqm')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f"woodlands_analysis.png")
    plt.close()
